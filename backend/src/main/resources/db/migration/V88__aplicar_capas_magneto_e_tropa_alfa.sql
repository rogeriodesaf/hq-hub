-- Reaplica as capas da Tropa Alfa aceitando números como "Volume 1" e "Vol. 2".
...
SET url_capa = CASE regexp_replace(edicao.numero, '[^0-9]', '', 'g')
    ...
    END,
    url_origem = CASE regexp_replace(edicao.numero, '[^0-9]', '', 'g')
...
WHERE serie.id = edicao.serie_id
  ...
  AND regexp_replace(edicao.numero, '[^0-9]', '', 'g') IN ('1', '2');
...
-- Magneto Triunfa não possui número na transcrição; vincula somente se houver
-- uma única edição com capa no cadastro Panini V1.
WITH candidatos AS (
    SELECT item.id AS item_id, min(edicao.id) AS edicao_id
    FROM itens_ordem_leitura item
    JOIN series serie
      ON lower(trim(serie.titulo)) = lower('X-Men: Magneto Triunfa')
     AND coalesce(serie.volume, 1) = 1
    JOIN editoras editora
      ON editora.id = serie.editora_id
     AND lower(trim(editora.nome)) = lower('Panini')
    ...
    WHERE lower(trim(item.titulo_referencia)) = lower('X-Men: Magneto Triunfa')
    GROUP BY item.id
    HAVING count(*) = 1
)
UPDATE itens_ordem_leitura item
