-- Vincula as capas das dez edicoes de Batman - 1a Serie, publicada pela Abril
-- entre 1984 e 1985. A migracao atualiza somente a serie existente.
...
WITH capas(numero, url_capa) AS (
    ...
    WHERE hqhub_normalizar_titulo_serie(editora.nome) LIKE 'abril%'
      AND (
          ...
          OR (
              hqhub_normalizar_titulo_serie(serie.titulo) = 'batman'
              AND coalesce(serie.volume, 1) = 1
              AND (
                  hqhub_normalizar_titulo_serie(coalesce(serie.descricao, '')) LIKE '%1 serie%'
                  OR hqhub_normalizar_titulo_serie(coalesce(serie.descricao, '')) LIKE '%primeira serie%'
              )
          )
      )
    ORDER BY serie.id
