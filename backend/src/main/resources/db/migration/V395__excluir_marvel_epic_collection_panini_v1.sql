-- Remove a serie cujo titulo inclui indevidamente Panini e V1, preservando suas edicoes na serie canonica.
CREATE OR REPLACE FUNCTION hqhub_mesclar_edicao_epic_collection_v1(descartada_id BIGINT, mantida_id BIGINT)
RETURNS VOID AS $$
BEGIN
    ...
    FOR incorreta IN
        SELECT serie.id
         ...
         WHERE coalesce(serie.volume, 1) = 1
           AND serie.id <> canonica_id
           AND hqhub_normalizar_titulo_serie(editora.nome) LIKE '%panini%'
           AND hqhub_normalizar_titulo_serie(serie.titulo) IN (
               hqhub_normalizar_titulo_serie('Marvel Epic Collection Panini'),
               hqhub_normalizar_titulo_serie('Marvel Epic Collection Panini V1')
           )
    LOOP
        FOR edicao_incorreta IN SELECT id, numero FROM edicoes WHERE serie_id = incorreta.id LOOP
            ...
            ELSE
                PERFORM hqhub_mesclar_edicao_epic_collection_v1(edicao_incorreta.id, edicao_canonica_id);
            END IF;
        END LOOP;
...
DROP FUNCTION hqhub_mesclar_edicao_epic_collection_v1(BIGINT, BIGINT);
