-- Reforca o cadastro da colecao para qualquer variante do nome da Editora Abril.
...
WHERE lower(trim(editora.nome)) LIKE '%abril%'
...
WITH referencias(numero, titulo, url_capa, url_origem) AS (VALUES
    ...
    WHERE lower(trim(editora.nome)) LIKE '%abril%' AND lower(trim(serie.titulo)) = lower('Melhor da Disney, O - As Obras Completas de Carl Barks') AND coalesce(serie.volume, 1) = 1
...
ON CONFLICT (serie_id, hqhub_normalizar_identidade(numero::text)) DO UPDATE
