-- Corrige o alvo cujo titulo no catalogo inclui o sufixo "Minisserie".
WITH capas(numero, isbn, url_capa, url_origem) AS (
    ...
    SELECT
        edicao.id,
        substring(trim(edicao.numero) from '([0-9]+)')::integer AS numero_numerico
    FROM edicoes edicao
    JOIN series serie ON serie.id = edicao.serie_id
    JOIN editoras editora ON editora.id = serie.editora_id
    WHERE hqhub_normalizar_titulo_serie(serie.titulo) IN (
              hqhub_normalizar_titulo_serie('Batman/Fortnite: Ponto Zero - Minisserie'),
              hqhub_normalizar_titulo_serie('Batman Fortnite: Ponto Zero - Minisserie')
          )
      AND hqhub_normalizar_titulo_serie(editora.nome) LIKE '%panini%'
      AND coalesce(serie.ano_inicio, 2021) = 2021
      AND substring(trim(edicao.numero) from '([0-9]+)') IS NOT NULL
)
