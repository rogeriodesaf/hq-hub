CREATE OR REPLACE FUNCTION hqhub_mesclar_edicao_saga(descartada_id BIGINT, mantida_id BIGINT)
RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hqhub_unificar_saga_volume(
    titulo_alvo TEXT,
    volume_alvo INTEGER,
...
BEGIN
                ...
                AND EXISTS (
                    SELECT 1
                     ...
                     WHERE ed.serie_id = s.id
                       AND hqhub_normalizar_identidade(ed.descricao) LIKE ('%' || volume_alvo || 'serie%')
                )
            )
       )
     ORDER BY
    ...
    LOOP
        ...
        LOOP
            ...
            ELSE
                PERFORM hqhub_mesclar_edicao_saga(edicao_descartada.id, edicao_mantida_id);
            END IF;
        END LOOP;
...
SELECT hqhub_unificar_saga_volume('Saga do Homem-Aranha, A', 1, 'Saga do Homem-Aranha, A');
SELECT hqhub_unificar_saga_volume('Saga do Homem-Aranha, A', 2, 'Saga do Homem-Aranha, A');
SELECT hqhub_unificar_saga_volume('Saga do Batman, A', 1, 'Saga do Batman, A');
SELECT hqhub_unificar_saga_volume('Saga do Batman, A', 2, 'Saga do Batman, A');

DROP FUNCTION hqhub_unificar_saga_volume(TEXT, INTEGER, TEXT);
DROP FUNCTION hqhub_mesclar_edicao_saga(BIGINT, BIGINT);
