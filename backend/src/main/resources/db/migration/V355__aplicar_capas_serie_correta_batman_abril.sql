-- Aplica as capas na serie correta, identificada em producao como
-- "Batman 1a Serie" (Abril), e cria a edicao 6 que faltou na importacao.
...
INSERT INTO edicoes (
    numero, titulo, nome_volume, fonte_externa, id_externo, url_origem,
    serie_id, data_criacao, data_atualizacao
)
SELECT
    '6', 'Batman 1ª Série #6', 'Batman 1ª Série #6', 'QUADRIKOMICS',
    'QUADRIKOMICS-BATMAN-ABRIL-1-SERIE-6',
    'https://quadrikomics.blogspot.com/2012/03/batman-1a-serie-abril.html',
    serie.id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM serie_alvo serie
WHERE NOT EXISTS (
    SELECT 1 FROM edicoes edicao
