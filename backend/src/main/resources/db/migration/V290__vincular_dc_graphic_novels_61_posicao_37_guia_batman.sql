-- Vincula a edicao 61 da DC Comics - Colecao de Graphic Novels (Eaglemoss)
-- a posicao 37 do guia cronologico do Batman.

WITH edicao_alvo AS (
    SELECT edicao.id, coalesce(
        nullif(trim(edicao.url_capa), ''),
        'https://spider145hqs.wordpress.com/wp-content/uploads/2022/08/dccomics_colecaodegraphicnovels_vol61_eaglemoss_06082022.jpg'
    ) AS url_capa
    FROM edicoes edicao
...
SET edicao_id = edicao.id,
    ...
    observacao = 'Vinculada a DC Comics - Colecao de Graphic Novels nº 61, da Eaglemoss.'
