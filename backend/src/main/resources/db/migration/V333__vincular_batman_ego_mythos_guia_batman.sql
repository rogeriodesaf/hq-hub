-- Vincula Batman - Ego, Mythos V1, à posição 48 do guia cronológico do Batman.
    ...
    WHERE hqhub_normalizar_titulo_serie(editora.nome) LIKE 'mythos%'
      ...
      AND (
          hqhub_normalizar_titulo_serie(serie.titulo)
              = hqhub_normalizar_titulo_serie('Batman - Ego')
...
SET edicao_id = alvo.id,
    titulo_referencia = 'Batman - Ego',
    detalhe_referencia = 'Mythos · V1',
    url_capa_referencia = alvo.url_capa,
    status_identificacao = 'CONFIRMADO',
    observacao = 'Vinculada a Batman - Ego, Mythos, V1.',
