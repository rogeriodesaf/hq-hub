CREATE OR REPLACE FUNCTION hqhub_descricao_saga_volume(descricao TEXT, volume_alvo INTEGER)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hqhub_mesclar_edicao_saga_robusta(descartada_id BIGINT, mantida_id BIGINT)
RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION hqhub_unificar_saga_robusta(titulo_alvo TEXT, volume_alvo INTEGER, titulo_canonico TEXT)
RETURNS VOID AS $$
DECLARE
...
BEGIN
     ...
     WHERE hqhub_normalizar_titulo_serie(s.titulo) = hqhub_normalizar_titulo_serie(titulo_alvo)
       ...
       AND (
            ...
            OR (
                s.volume IS NULL
                AND EXISTS (
                    SELECT 1 FROM edicoes ed
                     WHERE ed.serie_id = s.id
                       AND hqhub_descricao_saga_volume(ed.descricao, volume_alvo)
                )
            )
       )
     ORDER BY
    ...
    LOOP
        ...
        LOOP
            ...
            ELSE
                PERFORM hqhub_mesclar_edicao_saga_robusta(edicao_descartada.id, edicao_mantida_id);
            END IF;
        END LOOP;
...
SELECT hqhub_unificar_saga_robusta('Saga do Homem-Aranha, A', 1, 'Saga do Homem-Aranha, A');
SELECT hqhub_unificar_saga_robusta('Saga do Homem-Aranha, A', 2, 'Saga do Homem-Aranha, A');
SELECT hqhub_unificar_saga_robusta('Saga do Batman, A', 1, 'Saga do Batman, A');
SELECT hqhub_unificar_saga_robusta('Saga do Batman, A', 2, 'Saga do Batman, A');

DROP FUNCTION hqhub_unificar_saga_robusta(TEXT, INTEGER, TEXT);
DROP FUNCTION hqhub_mesclar_edicao_saga_robusta(BIGINT, BIGINT);
DROP FUNCTION hqhub_descricao_saga_volume(TEXT, INTEGER);
